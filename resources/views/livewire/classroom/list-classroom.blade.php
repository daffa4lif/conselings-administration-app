<div>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-2">
        <div class="w-full">
            <x-basic-label title="Jenis Kelas" />
            <select wire:model.live.debounce.400ms="filterMajor" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
    ...
    <div wire:loading.remove class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody>
                @forelse ($classrooms as $key => $item)
                <tr class="bg-white border-b hover:bg-gray-100">
                    <td class="px-6 py-4">
                        {{ ($classrooms->currentPage() - 1) * $classrooms->perPage() + $key + 1 }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $item->major }}
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        <a href="{{ route('classroom.detail', $item->id) }}" class="font-medium text-blue-600 hover:underline">Detail</a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-4 text-center text-gray-900 font-medium text-md">Tidak Ditemukan</td>
                </tr>
                @endforelse
            </tbody>
        </table>
