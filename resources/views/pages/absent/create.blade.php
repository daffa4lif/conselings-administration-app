...
<header>
    <x-main-header title="Buat Absent" />
    <x-breadcrumb :datas="[route('absent.index') => 'List Absent']" last="Buat Absent" />
</header>

<section class="w-full p-4 border border-gray-100 shadow rounded-lg mt-5">
    <h2 class="text-xl font-medium mb-2">Buat Absent</h2>
    ...
    <form method="POST">
        ...
        <div class="mt-20 mb-5">
            ...
            <select name="type" id="type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                <option @selected(old('type') == "IZIN")>IZIN</option>
                <option @selected(old('type') == "SAKIT")>SAKIT</option>
                <option @selected(old('type') == "ALPHA")>ALPHA</option>
            </select>
        </div>
        <div class="mb-5">
            <x-basic-label for="tanggal" title="Tanggal Kejadian" />
            <x-basic-input type="date" id="tanggal" name="date" value="{{ old('date') }}" required/>
        </div>
        <div class="mb-5">
            <x-basic-label for="keterangan" title="Keterangan" />
            <textarea name="description" id="keterangan" cols="30" rows="2" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" required>{{ old('description') }}</textarea>
        </div>
